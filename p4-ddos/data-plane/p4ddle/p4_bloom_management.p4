...
const bit<32> BLOOM_SIZE=32w32768;
...
control MyIngress(inout headers hdr,
    ...
    apply {
        ...
        bit<8> counter0_value;
        bit<8> counter1_value;
        bit<8> counter2_value;
        bit<8> counter3_value;
        bit<8> min_counter_value;
        ...
        if(sampling_counter_value == sampling_treshold_value) {
                ...
                hash(meta.index_fwd, HashAlgorithm.crc32, 32w0, {meta.srcAddr, meta.srcPort, meta.dstAddr, meta.dstPort, meta.ipProtocol}, BLOOM_SIZE);
                hash(meta.index_bwd, HashAlgorithm.crc32, 32w0, {meta.dstAddr, meta.dstPort, meta.srcAddr, meta.srcPort, meta.ipProtocol}, BLOOM_SIZE);
                hash(meta.index_fwd_short, HashAlgorithm.crc32, 32w0, {meta.srcAddr, meta.srcPort, meta.dstAddr, meta.dstPort}, BLOOM_SIZE);
                hash(meta.index_bwd_short, HashAlgorithm.crc32, 32w0, {meta.dstAddr, meta.dstPort, meta.srcAddr, meta.srcPort}, BLOOM_SIZE);

                if (block_of_registers_value == 0) {

                    flow_counter0.read(counter0_value, meta.index_fwd);
                    flow_counter0.read(counter1_value, meta.index_bwd);
                    flow_counter0.read(counter2_value, meta.index_fwd_short);
                    flow_counter0.read(counter3_value, meta.index_bwd_short);

                    if (counter0_value < counter1_value && counter0_value < counter2_value && counter0_value < counter3_value) {
                        min_counter_value = counter0_value; // variable1 is smallest
                    } else if (counter1_value < counter2_value && counter1_value < counter3_value) {
                        min_counter_value = counter1_value; // variable2 is smallest
                    } else if (counter2_value < counter3_value) {
                        min_counter_value = counter2_value; // variable3 is smallest
                    } else {
                        min_counter_value = counter3_value; // variable4 is smallest
                    }
                    ...
                    if(min_counter_value < 4){
                        ...
                        bit<64> ifb;
                        ignored_by_fulled_bloom0.read(ifb,0);
                        ignored_by_fulled_bloom0.write(0,ifb+1);
                    }
                    ...
                    flow_counter0.write(meta.index_fwd,counter0_value+1);
                    flow_counter0.write(meta.index_bwd,counter1_value+1);
                    flow_counter0.write(meta.index_fwd_short,counter2_value+1);
                    flow_counter0.write(meta.index_bwd_short,counter3_value+1);

                } // end block 0
                else {

                    flow_counter1.read(counter0_value, meta.index_fwd);
                    flow_counter1.read(counter1_value, meta.index_bwd);
                    flow_counter1.read(counter2_value, meta.index_fwd_short);
                    flow_counter1.read(counter3_value, meta.index_bwd_short);
                    ...
                    if(min_counter_value < 4){
                        ...
                        bit<64> ifb;
                        ignored_by_fulled_bloom1.read(ifb,0);
                        ignored_by_fulled_bloom1.write(0,ifb+1);
                    }
                    ...
                    flow_counter1.write(meta.index_fwd,counter0_value+1);
                    flow_counter1.write(meta.index_bwd,counter1_value+1);
                    flow_counter1.write(meta.index_fwd_short,counter2_value+1);
                    flow_counter1.write(meta.index_bwd_short,counter3_value+1);
