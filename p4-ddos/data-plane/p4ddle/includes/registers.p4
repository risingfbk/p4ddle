...
register<bit<8>>(BLOOM_SIZE) flow_counter0;
...
register<bit<8>>(BLOOM_SIZE) flow_counter1;
...
register<bit<64>>(1) ignored_by_fulled_bloom0;//just for test

register<bit<64>>(1) not_foldable_counter1;//just for test
register<bit<64>>(1) process_in_memory1;//just for test
register<bit<64>>(1) ignored_by_fulled_bloom1;//just for test
