...
def generate_and_store_packets_from_features(features,temp_dict,start_time_window,max_flow_len):
    ...
    number_of_packets = len(features[0]) # number of values for a single feature, AKA number of packets
...
def process_live_traffic(traffic_source, dataset_type, in_labels, max_flow_len, p4_compatible, switch_impl=None, register_bits=None, traffic_type='all',time_window=TIME_WINDOW):
    start_time = time.time()
    temp_dict = OrderedDict()
    labelled_flows = []
    ...
    if isinstance(traffic_source, pyshark.LiveCapture) == True:
        ...
        return labelled_flows, last_round
        ...
        process_in_memory = traffic_source.read_register("process_in_memory"+str(old_block),0)

        traffic_source.reset_register("not_foldable_counter"+str(old_block))
        traffic_source.reset_register("process_in_memory"+str(old_block))
        traffic_source.reset_all_registers(old_block)

        if switch_impl == "p4ddle":
            ignored_by_fulled_bloom = traffic_source.read_register("ignored_by_fulled_bloom"+str(old_block),0)
            traffic_source.reset_register("ignored_by_fulled_bloom"+str(old_block))
        ...
        packets_per_sample_sizes = list()

        for k in flows:
            num_of_packets = len(list(temp_dict[k].values())[0])
            k = list(k)
            k.append(round_counter)
            k.append(num_of_packets)
            packets_per_sample_sizes.append(k)

        apply_labels(temp_dict,labelled_flows,in_labels,traffic_type)

        if switch_impl == "p4ddle":
            return labelled_flows, processing, extraction, packets_per_sample_sizes, process_in_memory, total_incoming_packets, ignored_by_fulled_bloom, round_time, round_counter
        else:
            return labelled_flows, processing, extraction, packets_per_sample_sizes, process_in_memory, total_incoming_packets, round_time, round_counter
...
def main(argv):
    ...
    if args.dataset_type is None and args.dataset_folder is not None:
        print("Please specify the dataset type (IDS2017, IDS2018, DOS2019, and SYN2020)!")
        print(help_string)
